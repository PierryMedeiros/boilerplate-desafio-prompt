...
# 1. Trazer o prompt original (ruim) para análise
python src/pull_prompts.py

# 2. Rodar seus testes (inicialmente vão falhar ou passar dependendo do seu progresso)
pytest tests/test_prompts.py -v

# 3. Publicar sua versão otimizada no Hub
python src/push_prompts.py

# 4. Avaliar a performance com métricas de IA
...
Crie uma API Key: https://platform.openai.com/api-keys
...
Crie uma conta e uma API Key: https://smith.langchain.com/
...
Subir sua versão para o LangSmith:
...
│
├── requirements.txt
└── README.md                     # <--- VOCÊ EDITA (A Documentação)
📦 Entregáveis
...
O script push_prompts.py vai gerar um link público do seu prompt. Inclua-o no README.
